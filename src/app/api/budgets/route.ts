...
const fields = ["space_id", "category_id", "name", "amount", "period", "start_date", "end_date"] as const;
...
function budgetInput(body: unknown, partial: boolean) {
  ...
  if (!partial || "period" in body) {
    if (typeof body.period !== "string" || !periods.includes(body.period as typeof periods[number])) return "Invalid period";
  }
...
export async function POST(request: Request) {
  ...
  const { data, error } = await auth.supabase.from("budgets").insert(body as Record<string, unknown>).select().single();
