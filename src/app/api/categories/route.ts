...
export async function PATCH(request: Request) {
  ...
  const { data: existing, error: lookupError } = await auth.supabase
    ...
    .eq("id", id)
    .maybeSingle();
  if (lookupError) return apiError(400, "Unable to find category");
  if (!existing) return apiError(404, "Category not found");
  if (existing.is_system) return apiError(403, "System categories cannot be modified");
...
export async function DELETE(request: Request) {
  ...
  if (lookupError) return apiError(400, "Unable to find category");
  if (!existing) return apiError(404, "Category not found");
  if (existing.is_system) return apiError(403, "System categories cannot be deleted");
